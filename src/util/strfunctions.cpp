...
strip_comments (const std::string & item)
{
    ...
    result = trim(result, CFG66_TRIM_CHARS);
    return result;
}
 ...
 *
 * \param chars
 *      The set of characters to be trimmed.  Defaults to CFG66_TRIM_CHARS
 *      (" \t\n\v\f\r").  Another macro available is CFG66_TRIM_CHARS_QUOTES
 ...
 *
 * \param chars
 *      The set of characters to be trimmed.  The default is CFG66_TRIM_CHARS,
 ...
 *
 * \param delimiters
 *      The character(s) to use as separators for the tokens. Defaults to a
 *      Space character.
 *
 * \return
 ...
 * Method:
 *
 *  -#  Tokenize the string using CFG66_WHITE_CHARS = " \t\r\n\v\f" as
 *      the delimiters.
 *
 * \param source
 ...
 *
 * \param commentchar
 *      If non-zero (which is the default), then this character plus a
 *      space is prepended to each line.  A common value is "#", so that
 *      "# " is prepended..
 *
 * \return
