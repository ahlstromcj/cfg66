...
#include "platform_macros.h"            /* operating system/compiler macros */
    ...
    *destination = 0;                           /* empty out destination    */
    ...
    if (sourcelimit <= INT_MAX)                 /* just a sanity check      */
    {
        ...
        if (rcode == 0)
            ...
            util::warn_message("stringcopy truncation");
#else
        if (sourcelimit < destsize)              /* truncation impossible   */
        {
            destsize = sourcelimit + 1;
...
#endif  // defined PLATFORM_MING_OR_WINDOWS
...
/**
 *  Looks at an argv[0] string and determines the base location of the
 *  installation of the application.
 *
 * Include:
 *
 *      -   Windows
 *          -   C:/Qt/Qt5.12.9/5.12.9/mingw73_32/include
 *          -
 *      -   Linux
 *          -   /usr/include
 *          -   /usr/include/glib-2.0
 *          -   /usr/local/include/seq66
 *          -   /usr/lib/x86_64-linux-gnu/glib-2.0/include (minimal)
 *          -   /usr/share/cmake-3.16/include (minimal)
 *
 * Library:
 *
 *      -   Linux
 ...
 *
 *      -   Windows
 *          -   C:/Program Files (x86)/Seq66/data/share/doc (correct???)
 *      -   Linux
 *          -   /usr/local/share/doc/seq66
 *
 * Data (besides documents):
 *
 *      -   Windows
 *          -   C:/Program Files (x86)/Seq66/data
 *          -   C:/Program Files/Seq66/data
 *      -   Linux
 *          -   /usr/share/seq66 (icons, other data files)
 *          -   /usr/local/share/seq66 (icons, other data files)
 *
 *      For both, just append the specific data directory.
 *
 * To do:
 *
 *      -   UNC paths: "//server/name" or "\\server\name"
 *
 * \param arg0
...
/**
 *  This function cannot assume that the executable name is identical to the
 *  package name. For example, the executable is "qseq66", but the package is
 *  "seq66-0.99".
 *
 *      -   Windows
 *          -   C:/Program Files (x86)/Seq66/data
 ...
 *          -   /usr/local/share/seq66 (icons, other data files)
 *
 *  These are basically Seq66 conventions. Wish they were a little more
 *  uniform.
 *
 * \param arg0
 *      Source for the prefix directory only.
 *
 * \param pkgname
 *      Package subdirectory name. We cannot rely on the arg0 to get the
 *      package name, because the name of the executable might be different.
 *
 * \param subdir
 *      Optional add-on directory, such as data.
 */
